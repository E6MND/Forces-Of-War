...
public final class NativeProtocol {
    ...
    public static Exception getErrorFromResult(Intent resultIntent) {
        ...
        Bundle bridgeArgs = getBridgeArgumentsFromIntent(resultIntent);
        if (bridgeArgs == null || (errorBundle = bridgeArgs.getBundle("error")) == null) {
    ...
    public static int getLatestAvailableProtocolVersionForAction(Context context, String action, int minimumVersion) {
        return getLatestAvailableProtocolVersionForAppInfoList(context, actionToAppInfoMap.get(action), minimumVersion);
    }

    private static int getLatestAvailableProtocolVersionForAppInfoList(Context context, List<NativeAppInfo> appInfoList, int minimumVersion) {
        if (appInfoList == null) {
            return -1;
        }
    ...
    private static int getLatestAvailableProtocolVersionForAppInfo(Context context, NativeAppInfo appInfo, int minimumVersion) {
        Cursor c = null;
        try {
            c = context.getContentResolver().query(buildPlatformProviderVersionURI(appInfo), new String[]{PLATFORM_PROVIDER_VERSION_COLUMN}, (String) null, (String[]) null, (String) null);
            if (c == null) {
                return -1;
            }
            ...
            for (Integer knownVersion : KNOWN_PROTOCOL_VERSIONS) {
                if (knownVersion.intValue() < minimumVersion) {
                    ...
                    return -1;
                } else if (versions.contains(knownVersion)) {
                    int intValue = knownVersion.intValue();
                    if (c == null) {
                        return intValue;
                    }
                    c.close();
