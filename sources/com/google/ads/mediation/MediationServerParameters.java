        ...
        for (Map.Entry next : parameters.entrySet()) {
            ...
            if (field2 != null) {
                try {
                    ...
                    ev.D("Server option \"" + ((String) next.getKey()) + "\" could not be set: Illegal Access");
                } catch (IllegalArgumentException e2) {
                    ev.D("Server option \"" + ((String) next.getKey()) + "\" could not be set: Bad Type");
                }
            } else {
                ev.z("Unexpected server option: " + ((String) next.getKey()) + " = \"" + ((String) next.getValue()) + "\"");
            }
        }
