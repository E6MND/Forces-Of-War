        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    ...
                    childWidthSpec = View.MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, PageTransition.CLIENT_REDIRECT);
                } else {
                    childWidthSpec = View.MeasureSpec.makeMeasureSpec(lp.width, PageTransition.CLIENT_REDIRECT);
                }
                if (lp.height == -2) {
                    childHeightSpec3 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                } else if (lp.height == -1) {
                    childHeightSpec3 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, PageTransition.CLIENT_REDIRECT);
                } else {
                    childHeightSpec3 = View.MeasureSpec.makeMeasureSpec(lp.height, PageTransition.CLIENT_REDIRECT);
                }
                ...
                if (child2.getVisibility() != 8) {
                    ...
                    if (child2.getVisibility() != 8) {
                        ...
                        if (!canSlide || child2 == this.mSlideableView) {
                            if (lp2.weight > 0.0f) {
                                if (lp2.width != 0) {
                                    childHeightSpec = View.MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), PageTransition.CLIENT_REDIRECT);
                                    ...
                                    childHeightSpec = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, PageTransition.CLIENT_REDIRECT);
                                } else {
                                    childHeightSpec = View.MeasureSpec.makeMeasureSpec(lp2.height, PageTransition.CLIENT_REDIRECT);
                                }
                                if (canSlide) {
                                    int newWidth = widthAvailable - (lp2.leftMargin + lp2.rightMargin);
                                    int childWidthSpec2 = View.MeasureSpec.makeMeasureSpec(newWidth, PageTransition.CLIENT_REDIRECT);
                                    if (measuredWidth != newWidth) {
                                        child2.measure(childWidthSpec2, childHeightSpec);
                                    }
                                } else {
                                    child2.measure(View.MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp2.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), PageTransition.CLIENT_REDIRECT), childHeightSpec);
                                }
                            }
                        } else if (lp2.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp2.weight > 0.0f)) {
                            if (!skippedFirstPass) {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), PageTransition.CLIENT_REDIRECT);
                            } else if (lp2.height == -2) {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                            } else if (lp2.height == -1) {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, PageTransition.CLIENT_REDIRECT);
                            } else {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(lp2.height, PageTransition.CLIENT_REDIRECT);
                            }
                            child2.measure(View.MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, PageTransition.CLIENT_REDIRECT), childHeightSpec2);
                        }
                    }
                }
            }
