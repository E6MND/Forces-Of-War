...
public class ColorSuggestionListAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    private void setUpColorButton(View button, int index) {
        if (index >= this.mSuggestions.length) {
            ...
            button.setContentDescription((CharSequence) null);
        ...
        ((GradientDrawable) ((LayerDrawable) button.getBackground()).findDrawableByLayerId(R.id.color_button_swatch)).setColor(suggestion.mColor);
        String description = suggestion.mLabel;
        if (TextUtils.isEmpty(description)) {
            description = String.format("#%06X", new Object[]{Integer.valueOf(16777215 & suggestion.mColor)});
        }
