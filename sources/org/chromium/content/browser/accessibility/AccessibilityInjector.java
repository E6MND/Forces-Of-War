...
public class AccessibilityInjector extends WebContentsObserver {
    ...
    private final String mAccessibilityScreenReaderUrl = CommandLine.getInstance().getSwitchValue(ContentSwitches.ACCESSIBILITY_JAVASCRIPT_URL, DEFAULT_ACCESSIBILITY_SCREEN_READER_URL);
    ...
    public void injectAccessibilityScriptIntoPage() {
        if (accessibilityIsAvailable() && getAxsUrlParameterValue() == -1) {
            ...
            if (this.mContentViewCore.isDeviceAccessibilityScriptInjectionEnabled() && js != null && this.mContentViewCore.isAlive()) {
                ...
                this.mContentViewCore.getWebContents().evaluateJavaScript(js, (JavaScriptCallback) null);
    ...
    public void setScriptEnabled(boolean enabled) {
        ...
        if (accessibilityIsAvailable() && this.mInjectedScriptEnabled != enabled) {
            ...
            if (this.mContentViewCore.isAlive()) {
                this.mContentViewCore.getWebContents().evaluateJavaScript(String.format(TOGGLE_CHROME_VOX_JAVASCRIPT, new Object[]{Boolean.toString(this.mInjectedScriptEnabled)}), (JavaScriptCallback) null);
    ...
    private int getAxsUrlParameterValue() {
        ...
        try {
            for (NameValuePair param : URLEncodedUtils.parse(new URI(this.mContentViewCore.getWebContents().getUrl()), (String) null)) {
                if ("axs".equals(param.getName())) {
                    return Integer.parseInt(param.getValue());
                }
            }
            return -1;
        } catch (IllegalArgumentException | NumberFormatException | URISyntaxException e) {
            return -1;
        }
    }

    private String getScreenReaderInjectingJs() {
        return String.format(ACCESSIBILITY_SCREEN_READER_JAVASCRIPT_TEMPLATE, new Object[]{this.mAccessibilityScreenReaderUrl});
    }
    ...
    private static class TextToSpeechWrapper {
        ...
        public TextToSpeechWrapper(View view, Context context) {
            this.mView = view;
            this.mTextToSpeech = new TextToSpeech(context, (TextToSpeech.OnInitListener) null, (String) null);
            this.mSelfBrailleClient = new SelfBrailleClient(context, CommandLine.getInstance().hasSwitch(ContentSwitches.ACCESSIBILITY_DEBUG_BRAILLE_SERVICE));
        }
