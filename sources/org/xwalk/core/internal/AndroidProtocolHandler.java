...
class AndroidProtocolHandler {
    ...
    public static InputStream open(Context context, String url) {
        ...
        try {
            ...
            if (uri.getScheme().equals(FILE_SCHEME)) {
                ...
                if (!uri.getScheme().equals("app") || !uri.getHost().equals(context.getPackageName().toLowerCase()) || path.length() <= 1) {
                    return null;
                }
    ...
    public static Uri appUriToFileUri(Uri uri) {
        if (!$assertionsDisabled && !uri.getScheme().equals("app")) {
            ...
            try {
                return Uri.parse(new URI(FILE_SCHEME, SCHEME_SEPARATOR + nativeGetAndroidAssetPath() + APP_SRC + uri.getPath(), (String) null).normalize().toString());
    ...
    private static int getFieldId(Context context, String assetType, String assetName) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
        return context.getClassLoader().loadClass(context.getPackageName() + ".R$" + assetType).getField(assetName).getInt((Object) null);
    }
    ...
    private static InputStream openResource(Context context, Uri uri) {
        if (!$assertionsDisabled && !uri.getScheme().equals(FILE_SCHEME)) {
            ...
            if (!("/" + pathSegments.get(0) + "/").equals(nativeGetAndroidResourcePath())) {
    ...
    public static String getMimeType(Context context, InputStream stream, String url) {
        ...
        try {
            ...
            if ((uri.getScheme().equals("app") || (uri.getScheme().equals(FILE_SCHEME) && path.startsWith(nativeGetAndroidAssetPath()))) && (mimeType = URLConnection.guessContentTypeFromName(path)) != null) {
                return mimeType;
            }
            try {
